...
上游始终是 agent 形态，**没有纯问答的 ask 模式**。所以即便请求里没有任何工具，
问它「写一段 SVG」它也会把内容塞进一次「写文件」调用，而不是直接回答——
在聊天客户端里就表现为只回一句「我这就写…」然后没有下文。

代理会把这种调用的内容还原成回复正文（按扩展名包成对应语言的代码块），
所以纯聊天客户端能正常拿到代码。已知限制：内容是在上游把整个调用发完后
才一次性给出的，因此这类回答不是逐字流式的，长内容会先静默一段时间再整段出现。
