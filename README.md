...
除文本协议外，还会把上游**内置工具调用**翻译成客户端的工具。Cursor 的 agent 并不自己
执行工具，而是把「读文件」「跑命令」这类调用下发给客户端；这些帧原先被直接忽略，
表现就是「模型说要做某事，然后对话断了」。现在会按名称与 JSON Schema 映射到客户端
声明的对应工具（如 `read` / `bash` / `glob`）。用 `NATIVE_TOOL_BRIDGE=off` 可关闭。

**选模型很关键。** 实测能稳定完成多步骤 agent 任务的是 **`claude-4.5-sonnet`**
（创建文件 → 执行 → 汇报，连续多次成功）。
...
| 模型 | 表现 |
| --- | --- |
| `claude-4.5-sonnet` | 稳定可用 |
| `auto` / `default`、`composer-*` | Cursor 自家 agent 模型，只回一句「我这就去创建…」就结束 |
| `claude-4.6-opus-max` | 上游只下发「读文件」、从不下发写入，客户端会反复读同一个不存在的文件 |
| `gpt-5.1` | 同上，倾向宣告动作而不实际调用 |

遇到「只说不做」或反复读同一个文件，先换成 `claude-4.5-sonnet`。
