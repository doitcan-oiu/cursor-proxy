...
主要路径是**桥接上游的原生工具调用**。Cursor 的 agent 并不自己执行工具，而是把
「读文件」「写文件」「跑命令」「搜索」下发给客户端执行；这些调用原先被整个忽略，
表现就是「模型说要做某事，然后对话断了」。现在会解析出来并按名称与 JSON Schema
映射到客户端声明的对应工具（`read` / `write` / `bash` / `glob` 及常见命名变体）。
文本协议作为补充，覆盖上游没有内置对应物的自定义工具。用 `NATIVE_TOOL_BRIDGE=off` 可关闭桥接。
