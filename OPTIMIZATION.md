...
### ~~P1：「宣告了没做完」告警刷屏，还往用户回复里插说明~~（已在 Go 版修复）
上一条修复加的收尾提示实际使用中十小时内触发了 33 次，绝大多数所在的轮次其实好好的。
两个原因：

1. **完成帧到了却被丢弃**。`parseNativeToolCall` 挑中工具字段后，只要解析器因为
   缺字段返回 nil 就整个返回 nil——这次调用凭空消失，既没下发给客户端，也没被
   标记成已完成，收尾时又被当成「宣告了没做完」。日志里那些 `write_file`
   多半是这么来的。改成：完成帧存在就一定给出一次调用，参数解析不出来时
   退化成带规范名的未识别调用，至少客户端知道发生过什么。
2. **判定过宽**。模型宣告了工具又放弃很常见——想了想改主意，或换个 id 重发。
   只要这一轮给出了正文或完成了别的工具调用，这种半途而废就是无害的中间状态。
   改成只在「这一轮什么都没产出」时才提示，那正是当初要解决的场景
   （只回一句「我这就去搜索」然后静默结束）。
